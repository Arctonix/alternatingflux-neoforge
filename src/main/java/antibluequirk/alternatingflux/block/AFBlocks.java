 ...
 * Stable v1.0.5: AF tier only — UAF relay/transformer blocks removed.
 *
 * Relays: reuse IE EnergyConnectorBlockEntity with voltage="AF". IE 9.x
 * exposes a public constructor EnergyConnectorBlockEntity(String voltage, boolean
 * relay, BlockPos, BlockState) and the same SPEC_TO_TYPE / NAME_TO_SPEC public-static
 * maps for injection. We register our own BE types and blocks, then push AF into those
 * maps during common setup. The private LENGTH map (wire anchor height) is poked via
 * reflection.
 *
 * Transformer: AFTransformerBlock extends IE ConnectorBlock; the block entity extends
 * IE TransformerBlockEntity with HV as the low-side wire.
 *
